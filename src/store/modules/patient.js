...
const getters = {
  ...
  getFilteredPatients: (state) => state.filteredPatients,
...
const actions = {
  ...
  getPatient() {},
  listPatients({ commit, getters }, { search, page = 1, limit = 20 }) {
  ...
  createPatientAddress(
    { commit },
    { patient, address, address_2, city, state, country, zipcode }
  ) {
    return new Promise((resolve, reject) => {
      this.$axios
        .post(`/api/patient/${patient.id}/addresses/`, {
          address,
          address_2,
          city,
          state,
...
const mutations = {
  ...
  setFilteredPatients(state, data) {
  ...
  REMOVE_PATIENT_CARD(state, data) {
    if (state.selectedRecord) {
      const findRowIndex = state.rows.findIndex(
        ({ id }) => id === state.selectedRecord.id
      );
      if (findRowIndex > -1) {
        const findCardIndex = state.rows[findRowIndex].customer_card.findIndex(
          ({ id }) => id === data.id
        );
        if (findCardIndex > -1) {
